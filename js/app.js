...
async function loadSession(user){
  ...
  if(lastLogin!==today){localStorage.setItem('creatorhub_lastLogin',today);var xpRes=await sb.rpc('award_xp',{p_amount:25,p_reason:'每日登录'});if(!xpRes.error&&xpRes.data){myXP=xpRes.data.xp||myXP+25;myLevel=xpRes.data.level||Math.floor(myXP/100)+1;}}
...
async function doRegister(){
  ...
  var allowed=await sb.rpc('is_username_allowed',{p_username:user});
  if(allowed.error){err.textContent='注册失败，请稍后重试';return;}
  if(!allowed.data){err.textContent='该用户名不允许使用';return;}
  ...
  if(_r.error){var msg=_r.error.message||'';err.textContent=msg==='User already registered'?'该邮箱已被注册':(/username|duplicate|profiles_username_key|unique/i.test(msg)?'用户名已被占用':msg);return;}
...
async function saveXP(amount,reason){
  ...
  var r=await sb.rpc('award_xp',{p_amount:amount,p_reason:reason});
  if(r.error){showLoadError(r.error);return;}
  if(r.data){myXP=r.data.xp||myXP+amount;myLevel=r.data.level||Math.floor(myXP/100)+1;}
  updateLevelDisplay();
}
...
// Avatar & XP modules extracted to js/components/avatar.js and js/xp.js
// ========================================
// DATA
// ========================================
...
function saveProfileField(field,value){if(field==='xp'||field==='level'||field==='trust_score')return;var update={};update[field]=value;sb.from('profiles').update(update).eq('username',currentUser).then(function(r){if(r.error)showLoadError(r.error);});}
...
async function addXPForUser(username,amount,reason){if(username!==currentUser)return;var r=await sb.rpc('award_xp',{p_amount:amount,p_reason:reason});if(r.error){showLoadError(r.error);return;}if(r.data){myXP=r.data.xp||myXP+amount;myLevel=r.data.level||Math.floor(myXP/100)+1;updateLevelDisplay();}}
async function processInviteCode(){var inviteCode=sessionStorage.getItem('_invite_code');if(!inviteCode||!currentUser)return;sessionStorage.removeItem('_invite_code');var r=await sb.rpc('redeem_invite_code',{p_code:inviteCode});if(r.error){showLoadError(r.error);return;}if(r.data&&r.data.status==='ok')showToast('已通过邀请加入，双方已自动成为好友','success');}
